...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('undername_hbls', function (Blueprint $table) {
            $table->id();
            $table->string('code_undername_hbl', 3)->unique();
            $table->string('name');
    ...
    {
        Schema::table('undername_hbls', function (Blueprint $table) {
            $table->dropForeign(['mandatory_tax_id']);
            $table->dropForeign(['user_id']);
        });
        Schema::dropIfExists('undername_hbls');
    }
};
